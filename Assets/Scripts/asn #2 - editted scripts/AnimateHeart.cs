...
public class AnimateHeart : MonoBehaviour
{
    ...
    static int numSphere = 360; //number of spheres we wanna create
    float time = 0f;
    Vector3[] initPos; 
    Vector3[] newScale;
    ...
    {
        time += Time.deltaTime; 
        //the time variable holds a reference to the interval (sec) from the last frame to the current one
