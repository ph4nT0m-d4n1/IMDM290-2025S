...
public class Lerp_Heart : MonoBehaviour
{
    GameObject[] spheres;
    ...
    void Update()
    {
        ...
        for (int i = 0; i < numSphere; i++){
            ...
            lerpFraction = Mathf.Cos(time) * -0.5f + 0.5f;
            ...
            if (i % 2 == 0)
            {
                // startPosition[i] += new Vector3(0.01f, 0.01f, 0f);
                endPosition[i] += new Vector3(0.001f, 0.001f, 0f);

                Color color = Color.HSVToRGB(Mathf.Abs(hue * Mathf.Cos(time)), Mathf.Cos(time), 2f + Mathf.Cos(time));
                sphereRenderer.material.color = color;
            }
            else
            {
                ...
                Color color = Color.HSVToRGB(Mathf.Abs(hue * Mathf.Sin(time)), Mathf.Sin(time), 2f + Mathf.Sin(time));
                sphereRenderer.material.color = color;
            }
        }
    }
